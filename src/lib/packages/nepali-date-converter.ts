import { convertToAD, convertToBS, format, Language, parse } from './nepali-date-helper';
import type { IYearMonthDate, IAdBs } from './nepali-date-helper';
...
export default class NepaliDate {
	...
	constructor(...args: (string | number | Date)[]) {
		const constructorError = new Error('Invalid constructor args');
		if (args.length === 0) {
			...
			const monthIndex = args[1] as number;
			const date = args[2] as number;
			this[setDayYearMonth](year, monthIndex, date);
			this[convertToADMethod]();
		} else {
			throw constructorError;
		}
	...
	private [setDayYearMonth](year: number, month = 0, date = 1, day = 0) {
